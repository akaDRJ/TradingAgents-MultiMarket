...
- treat local market integrations like Magisk-style modules: upstream core stays close to stock, modules expose a manifest and register hooks
...
Extension modules are mounted through `tradingagents.extensions.market_ext`.
Each module exposes `get_extension_module()` and can provide market matching,
data routing, prompt context, analyst-specific instructions, and optional
prebuilt reports. The default mounted modules are:

```text
tradingagents.extensions.ashare,tradingagents.extensions.crypto
```

Override them with `TRADINGAGENTS_EXTENSION_MODULES` to add, remove, or reorder
modules without editing the upstream graph, agents, or dataflow code.
